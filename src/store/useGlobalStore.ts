...
export const useGlobalStore = create<GlobalStoreState>()((set, get) => ({
  ...
  addEvent: (newEvent: EventType) => {
    set((state) => {
      const updatedUserInfo = state.userInfo
        ? {
            ...state.userInfo,
            hostedEvents: [...(state.userInfo?.hostedEvents || []), newEvent],
          }
        : null;
  ...
  loadInterests: async () => {
    ...
    try {
      ...
      if (interestRes && !interestRes.error) {
        ...
        console.error("Erreur lors du fetch des intérêts:", interestRes?.error);
      }
    } catch (error) {
  ...
  loadUsers: async (userId: string, token: string) => {
    ...
    try {
      const usersRes = await fetchData(endpoint, HttpMethod.GET, null, token);
